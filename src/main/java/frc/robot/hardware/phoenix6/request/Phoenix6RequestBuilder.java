...
public class Phoenix6RequestBuilder {
	...
	public static Phoenix6Request<Rotation2d> buildBangBangRequest(Supplier<Rotation2d> currentVelocity) {
		...
		return new Phoenix6Request<>(
			Rotation2d.kZero,
			dutyCycleOut,
			(Rotation2d targetVelocity) -> dutyCycleOut
				.withOutput(bangBangController.calculate(currentVelocity.get().getRotations(), targetVelocity.getRotations()) == 0 ? -0.65 : 0.65)
		);
	}
