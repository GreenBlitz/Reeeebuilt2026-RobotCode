...
public class AutosBuilder {
	...
	private static Supplier<PathPlannerAutoWrapper> getLQuarterAuto(
		Robot robot,
		Supplier<Command> resetSubsystems,
		Supplier<Command> openIntake,
		Supplier<Command> closeIntake,
				...
				new ParallelCommandGroup(
						PathFollowingCommandsBuilder
								.followAdjustedPathThenStop(
										robot.getSwerve(),
										() -> robot.getPoseEstimator().getEstimatedPose(),
										startingSide == AllianceSide.DEPOT ? PathHelper.PATH_PLANNER_PATHS.get("L quarter light") : PathHelper.PATH_PLANNER_PATHS.get("R quarter light"),
										pathfindingConstraints,
										regularIsNearEndOfPathTolerance,
										stuckIsNearEndOfPathTolerance,
										stuckDebounceSeconds,
	...
	private static Supplier<PathPlannerAutoWrapper> getQuarterAutoWithOtherFeeder(
		Robot robot,
		Supplier<Command> resetSubsystems,
		Supplier<Command> openIntake,
		Supplier<Command> closeIntake,
		...
		return () -> new PathPlannerAutoWrapper(
			...
			new Pose2d(),
			startingSide == AllianceSide.OUTPOST ? "R quarter" : "L quarter to outpost"
		);
	}
