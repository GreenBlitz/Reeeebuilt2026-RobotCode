...
public class Swerve extends GBSubsystem {
	...
	public OdometryData[] getAllOdometryData() {
		...
		for (int i = 0; i < odometryData.length; i++) {
			odometryData[i] = new OdometryData(
				...
				imu instanceof EmptyIMU ? Optional.empty() : Optional.of(imuSignals.rollSignal().asArray()[i]),
				imu instanceof EmptyIMU ? Optional.empty() : Optional.of(imuSignals.pitchSignal().asArray()[i]),
	...
	public boolean isOnBump() {
		return imuSignals.rollSignal().isGreater(SwerveConstants.ROLL_ON_BUMP.plus(SwerveConstants.ROLL_ON_BUMP_TOLERANCE))
			|| imuSignals.pitchSignal().isGreater(SwerveConstants.PITCH_ON_BUMP.plus(SwerveConstants.PITCH_ON_BUMP_TOLERANCE));
	}
